...
class Account(dict):
    """ This class allows to easily access Account data
        ...
        :rtype: dictionary
        :raises graphene.exceptions.AccountDoesNotExistsException: if account does not exist
        ...
        .. code-block:: python
            ...
            from graphene.account import Account
    ...
    def refresh(self):
        ...
        """
        import re
        if re.match(r"^1\.2\.[0-9]*$", self.name):
        ...
        if self.full:
            account = self.graphene.rpc.get_full_accounts([account["id"]], False)[0][1]
    ...
    def balances(self):
        """ List balances of an account. This call returns instances of
            :class:`graphene.amount.Amount`.
        """
        from .amount import Amount
        balances = self.graphene.rpc.get_account_balances(self["id"], [])
        return [
            Amount(b, graphene_instance=self.graphene)
    ...
    def callpositions(self):
        ...
        if not self.full:
            ...
            self.refresh()
        from .dex import Dex
        dex = Dex(graphene_instance=self.graphene)
        return dex.list_debt_positions(self)

    @property
    def call_positions(self):
        """ Alias for :func:graphene.account.Account.callpositions
        """
    ...
    def history(
        ...
        """
        from graphsdkbase.operations import getOperationNameForId
        _limit = 1000
        cnt = 0

        mostrecent = self.graphene.rpc.get_account_history(
            self["id"],
            "1.11.{}".format(0),
            1,
        ...
        while True:
            # RPC call
            txs = self.graphene.rpc.get_account_history(
                self["id"],
                "1.11.{}".format(last),
                _limit,
                "1.11.{}".format(first - 1),
    ...
    def upgrade(self):
        return self.graphene.upgrade_account(account=self)


class AccountUpdate(dict):
    """ This purpose of this class is to keep track of account updates
        as they are pushed through by :class:`graphene.notify.Notify`.
    ...
    def __init__(
        ...
        if isinstance(data, dict):
            ...
            account = Account(data, graphene_instance=self.graphene)
            update = self.graphene.rpc.get_objects([
    ...
    def account(self):
        """ In oder to obtain the actual
            :class:`graphene.account.Account` from this class, you can
