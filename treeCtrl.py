...
class WalletFrame(wx.Frame):
    ...
    def InitUI(self):
        ...
        api_cmd_box = wx.BoxSizer(wx.HORIZONTAL)
        vbox = wx.BoxSizer(wx.VERTICAL)
        api_panel = wx.Panel(self, -1)
        command_panel = wx.Panel(self, -1)
        
        self.tree = wx.TreeCtrl(api_panel, 1, wx.DefaultPosition, (-1, -1), 
                                wx.TR_HIDE_ROOT|wx.TR_HAS_BUTTONS)
        root = self.tree.AddRoot('api')

        common_api = self.tree.AppendItem(root, '常用')
        wallet_api = self.tree.AppendItem(root, 'wallet')
        chain_api = self.tree.AppendItem(root, 'chain')
        self.tree.AppendItem(common_api, 'info')
        self.tree.AppendItem(common_api, 'transfer')
        self.tree.AppendItem(common_api, 'get_object')
        self.tree.AppendItem(common_api, 'get_account')
        self.tree.AppendItem(common_api, 'account_balances')

        self.tree.AppendItem(wallet_api, 'create_wallet')
        self.tree.AppendItem(wallet_api, 'lock')
        self.tree.AppendItem(wallet_api, 'unlock')
        self.tree.AppendItem(wallet_api, 'transfer')

        self.tree.AppendItem(chain_api, 'create_wallet')
        self.tree.AppendItem(chain_api, 'lock')
        self.tree.AppendItem(chain_api, 'unlock')
        self.tree.AppendItem(chain_api, 'transfer')

        self.tree.Bind(wx.EVT_TREE_SEL_CHANGED, self.OnSelChanged, id=1)
        self.display = wx.StaticText(command_panel, -1, '', (10, 10), style=wx.ALIGN_CENTER)
        vbox.Add(self.tree, 1, wx.EXPAND)
        api_cmd_box.Add(api_panel, 1, wx.EXPAND)
        api_cmd_box.Add(command_panel, 1, wx.EXPAND)
        api_panel.SetSizer(vbox)
        self.SetSizer(api_cmd_box)
        self.Center()
        self.Show(True)
